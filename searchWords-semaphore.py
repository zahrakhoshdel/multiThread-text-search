...
def thread_function(sem, index, NTLine, string_to_search, file):
...
if __name__ == "__main__":
    ...
    #TLines = (NLines/THREAD_NUMBER)

    fSize = os.stat(filename).st_size
    chunk = math.ceil((fSize / THREAD_NUMBER))
    print("File size of the source file: \n", fSize)
    #print('chunk: ', chunk)
    ...
    sem = threading.Semaphore(THREAD_NUMBER)
    threads = list()
    for index in range(THREAD_NUMBER):

        #logging.info("Main    : create and start thread %d.", index)
        x = threading.Thread(target=thread_function, args=(sem,
